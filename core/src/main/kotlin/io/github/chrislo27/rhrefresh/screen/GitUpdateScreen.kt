...
class GitUpdateScreen(main: RHREfreshApplication) : ToolboksScreen<RHREfreshApplication, GitUpdateScreen>(main) {
    ...
    init {
        ...
        stage.bottomStage.elements += object : Button<GitUpdateScreen>(palette, stage.bottomStage,
            ...
            override fun onLeftClick(xPercent: Float, yPercent: Float) {
                ...
                Gdx.net.openURI(if (repoStatus == RepoStatus.NO_INTERNET_CANNOT_CONTINUE) RHREfresh.GITHUB_RELEASES else RHREfresh.DATABASE_RELEASES)
            }
...
//                this.fontScaleMultiplier = 0.9f
    ...
    fun fetch() {
        ...
        coroutine = GlobalScope.launch {
            ...
            try {
                if ((!RHREfresh.forceGitFetch && RHREfresh.DATABASE_BRANCH != RHREfresh.DEV_DATABASE_BRANCH) || RHREfresh.forceGitCheck) {
                    label.text = Localization["screen.database.checkingGithub"]
                    try {
                        val current = JsonHandler.fromJson<SfxDbInfoObject>(RHREfreshApplication.httpClient.prepareGet(RHREfresh.DATABASE_CURRENT_VERSION).execute().get().responseBody)
                        ...
                        if (ver > RHREfresh.VERSION) {
                            ...
                            Toolboks.LOGGER.info("Incompatible SFXDB versions: requires ${current.requiresVersion}, have ${RHREfresh.VERSION}")
    ...
    override fun renderUpdate() {
        ...
        if ((Gdx.input.isKeyJustPressed(Input.Keys.ENTER)
                ...
                || repoStatus == RepoStatus.DONE || (RHREfresh.DATABASE_BRANCH == RHREfresh.DEV_DATABASE_BRANCH
