package io.github.chrislo27.rhrefresh.sfxdb
...
import io.github.chrislo27.rhrefresh.git.SfxDbInfoObject
...
import io.github.chrislo27.rhrefresh.sfxdb.datamodel.impl.PitchDependent
import io.github.chrislo27.rhrefresh.sfxdb.datamodel.impl.special.EndRemix
import io.github.chrislo27.rhrefresh.sfxdb.datamodel.impl.special.PlayalongModel
import io.github.chrislo27.rhrefresh.sfxdb.json.GameObject
import io.github.chrislo27.rhrefresh.sfxdb.json.mapToDatamodel
import io.github.chrislo27.rhrefresh.sfxdb.json.toJsonObject
import io.github.chrislo27.rhrefresh.soundsystem.AudioPointer
import io.github.chrislo27.rhrefresh.soundsystem.Derivative
import io.github.chrislo27.rhrefresh.soundsystem.SampleID
...
object SFXDatabase : Disposable {
    ...
    val CUSTOM_SFX_FOLDER: FileHandle by lazy {
        RHREfresh.RHREFRESH_FOLDER.child("customSounds/")
    }
    val CUSTOM_MODDING_METADATA_FOLDER: FileHandle by lazy {
        RHREfresh.RHREFRESH_FOLDER.child("customModdingMetadata/")
    }
    ...
    class SFXDBData : Disposable {
        ...
        val sfxCreditsLegacy: List<String>
        ...
        private val folders: List<SFXDirectory> by lazy {
            ...
            CUSTOM_SFX_FOLDER.child("README_SFX.txt").writeString(CustomSoundNotice.getNotice(), false,
                                                                  "UTF-8")
            val custom = if (RHREfresh.disableCustomSounds) emptyList() else CUSTOM_SFX_FOLDER.list { fh ->
                fh.isDirectory
            }.mapNotNull {
                ...
                val sfx = it.list { file: File ->
                    file.extension in RHREfresh.SUPPORTED_DECODING_SOUND_TYPES
                }
        ...
        private val sfxCreditsLegacyFh: FileHandle by lazy {
            GitHelper.SOUNDS_DIR.child("credits.json")
        }
        private val sfxCreditsFh: FileHandle by lazy {
            GitHelper.SOUNDS_DIR.child("credits-refresh.json")
        }
        ...
        init {
            ...
            sfxCreditsLegacy = sfxCreditsLegacyFh.takeIf(FileHandle::exists)?.readString("UTF-8")?.let {
                try {
            ...
            if (editorVersion > RHREfresh.VERSION)
                error("SFX database version ($editorVersion) is higher than this RHRE version (${RHREfresh.VERSION})")
        }
...
//            }
            ...
            if (datajsonFile.exists()) {
                ...
                sfxList.forEach { fh ->
                    ...
                    game.objects += Cue(game, "${game.id}/$name", listOf(), name, "", CustomSoundNotice.DURATION,
                                        ...
                                        earliness = 0f, loopStart = 0f, loopEnd = 0f, pitchBending = false, writtenPitch = 0, randomPitchLowCents = 0, randomPitchHighCents = 0)
                }

                if (RHREfresh.outputCustomSfx) {
        ...
        private fun addGameAndObjects(game: Game) {
            ...
            if (existingGame != null) {
                if (isOverwriting) {
                    ...
                    if (game.isSpecial && !RHREfresh.EXPERIMENTAL)
        ...
        private fun addSpecialGeneratedGames() {
            ...
            val playalongGame = Game(PLAYALONG_GAME_ID, "Playalong Input Entities", specialGame.series,
                                     ...
                                     null, "Playalong", false, specialGame.priority, -4,
